...
import * as sessionsSchemas from "$dbSchemas/Sessions"
import { eq, inArray, or, and, sql } from "drizzle-orm"
import PostgresDB from "./PostgresDB"

type SessionStore = FastifySessionStore & {
  destroyAllUserOrganizationSessions: (options: { userId: number, organizationId: number }) => Promise<void>
}
...
export default class DrizzleSessionStore {
  ...
  store: SessionStore = {
    ...
    destroyAllUserOrganizationSessions: async(options: { userId: number, organizationId: number }) => {
      ...
      const sessions = await this.db
        ...
        .where(
          and(
            or(
              eq(sessionsSchemas.sessions.authenticatedUserId, userId),
              eq(sessionsSchemas.sessions.callerUserId, userId),
            ),
            eq(
              sql`data->'data'->>'selectedOrganizationId'`,
      ...
      await Promise.all(
        sessions.map((session) => this.dropSession(session.sid))
      )
    },
    ...
    set: (sid, session, callback) => {
      ...
      const authenticatedUserId = data.data.authenticatedUserId
  ...
  dropUsersSessions = async (ids: number | number[]) => {
    ...
    await this.db
      ...
      .where(
        or(
          inArray(sessionsSchemas.sessions.authenticatedUserId, userIds),
