...
declare global {
  interface Window {
    openDropdown: (dropdownId: string, referenceComponent?: HTMLElement) => void
    closeDropdown: (dropdownId: string) => void
    toggleDropdown: (e: MouseEvent, dropdownId: string) => void
...
type DropdownHorizontalPosition = (typeof _HORIZONTAL_POSITIONS)[number]
type DropdownVerticalPosition = (typeof _VERTICAL_POSITIONS)[number]

const DEFAULT_POSITION: DropdownPosition = "bottom"

const getDropdownOptions = (element: HTMLElement) => {
  const position = ALLOWED_POSITIONS.includes(
...
const handleHorizontalPosition = (
  toggleRect: DOMRect,
  elRect: DOMRect,
  position: DropdownVerticalPosition,
) => {
  const pos = position.split("-")[1] || "center"
  switch (pos) {
    default:
    case "center":
      return toggleRect.left + toggleRect.width * 0.5 - elRect.width * 0.5
    case "right":
      return toggleRect.left + toggleRect.width - elRect.width
    case "left":
      return toggleRect.left
  }
}
...
const handleVerticalPosition = (
  toggleRect: DOMRect,
  elRect: DOMRect,
  position: DropdownHorizontalPosition,
) => {
  const pos = position.split("-")[1] || "center"
  switch (pos) {
    default:
...
const setDropdownPosition = (
  ...
  const { position, inheritWidth, relative, relativeElement, offset } =
  ...
  switch (position) {
    ...
    case "bottom-left":
    case "bottom-right":
      top = toggleTop + toggleHeight + offset
      viewportTop = toggleTop + toggleHeight + offset
      left = handleHorizontalPosition(toggleRect, elRect, position)
      viewportLeft = left
      break
    case "top":
    case "top-left":
    case "top-right":
      top = toggleTop - elHeight - offset
    ...
    case "right-top":
    case "right-bottom":
      left = toggleLeft + toggleWidth + offset
    ...
    case "left-top":
    case "left-bottom":
      left = toggleLeft - elWidth - offset
...
window.openDropdown = (
  ...
  if (dropdown) {
    if (dropdown.getAttribute("data-permanently-open") === "true") {
      return
    }
    ...
    const ref = referenceComponent
      ? referenceComponent
      : dropdown.getAttribute("data-reference")
        ? (document.getElementById(
            dropdown.getAttribute("data-reference") as string,
          ) as HTMLElement)
...
window.toggleDropdown = (e: MouseEvent, dropdownId: string) => {
  ...
  const dropdown = document.getElementById(dropdownId)
  if (dropdown) {
    const isOpen = dropdown.hasAttribute("data-clone")
      ? !!document.getElementById(dropdownId + "-clone")
...
window.onDropdownResize = () => {
  ...
  dropdowns.forEach((dropdown) => {
    ...
    const ref = dropdown.getAttribute("data-reference")
      ? (document.getElementById(
          dropdown.getAttribute("data-reference") as string,
        ) as HTMLElement)
      : toggle
...
window.closeDropdowns = (container = document.body, exclude?: string) => {
  ...
  dropdowns.forEach((dropdown) => {
    if (dropdown.id !== exclude) {
      if (
        dropdown.classList.contains("dropdown--open") ||
        document.querySelector(`#${dropdown.id}-clone`)
      ) {
