...
const addTooltip = (element: HTMLElement): void => {
  if (isOverflowing(element) || element.hasAttribute("x-chunk-id")) {
    const currentValue = element.getAttribute("data-tooltip-text")
    if (currentValue) {
      element.setAttribute("data-tooltip-text", currentValue)
    } else {
