...
export const generateSorterJSONSchema = (
...
let i = 0
export const generateSortersSchema = (
  acceptedSorters: PgColumn[],
...
export const sortersToDrizzleOrderBy = (options: {
  ...
  const sortersArray: MonoSorter[] = Array.isArray(options.sorters)
    ? options.sorters
    : [options.sorters]
  return sortersArray.map((sorter) => {
    ...
    if (!column) {
      throw new Error(
        ...
        `Error during sorting parsing: field ${sorter} not found in schema`,
      )
    }
