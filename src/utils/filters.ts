...
const simpleFilterToDrizzleWhere = (
  filter: IngressSimpleFilter,
  dbSchema: DbSchema,
): SQL => {
  const normalizedValue =
    filter.operator === "in" || filter.operator === "not in"
      ? filter.values
      : (filter as IngressSimpleFilterMonoValue).value
  const operatorFn = operatorMapping[filter.operator] as (
    column: PgColumn,
...
const digFilter = (
  ...
  if ("connector" in inputFilter) {
    ...
    if (!dbSchema) {
      throw new Error(
        `Error during filter parsing: field ${filter.field} not found in schema`,
      )
    }
...
export const filterToDrizzleWhere = (options: {
  ...
  return and(...startFilters.map((f) => digFilter(f, schemas)))
}
...
const getSingleSimpleLeafJSONSchema = (
  filterSchema: PgColumn,
): JSONSchema7[] => {
  const coreFilterSchema = pgTypeToJsonType(filterSchema)

  const generateJSONSchema = (options: {
    operators: string[]
    multiple: boolean
  }) =>
  ...
  if (coreFilterSchema.type === "string") {
    ...
    return [
      generateJSONSchema({
        operators: [...EQUALITY_OPERATORS, ...LIKE_OPERATORS],
        multiple: false,
      }),
      generateJSONSchema({ operators: INCLUSION_OPERATORS, multiple: true }),
    ]
...
const getFilterSimpleLeafJSONSchema = (
...
const getFilterLogicLeafJSONSchema = (
  _acceptedFilters: PgColumn[],
  id: string,
): JSONSchema7 => {
  ...
  return {
    ...
    oneOf: [
      ...
      { $ref: logicalLeafId },
      {
        ...
        type: "array",
        items: {
          anyOf: [{ $ref: simpleLeafId }, { $ref: logicalLeafId }],
        },
      },
    ],
  }
